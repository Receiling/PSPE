...
class StoreLoggingLevelAction(configargparse.Action):
    """StoreLoggingLevelAction converts string into logging level
    """
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        """__call__ gets the key `value` in the LEVELS, or just uses value
        """
...
class CheckPathAction(configargparse.Action):
    """CheckPathAction checks file path, if not exits, then create dir (file)
    """
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        """__call__ checks file path, if not exits, then create dir(file)
        """
