...
def flatten_and_batch_shift_indices(indices, sequence_length):
    ...
    if torch.max(indices) >= sequence_length or torch.min(indices) < 0:
        raise RuntimeError(
            "All elements in indices should be in range (0, {})".format(sequence_length - 1))
...
def pad_vecs(vecs, padding_size, dtype, device):
    ...
    if max_length == 0:
        pad_vecs = torch.cat(
            [get_padding_vector((1, padding_size), dtype, device).unsqueeze(0) for _ in vecs], 0)
    ...
    for vec in vecs:
        pad_vec = torch.cat(
            vec + [get_padding_vector((1, padding_size), dtype, device)] * (max_length - len(vec)),
            0).unsqueeze(0)
        ...
        assert pad_vec.size() == (1, max_length,
...
def get_bilstm_minus(batch_seq_encoder_repr, span_list, seq_lens):
    ...
    assert len(span_list) == len(
        seq_lens), "the length of span list is not equal to batch seq lens's length"
    ...
    for seq_encoder_repr, (s, e), seq_len in zip(batch_seq_encoder_repr, span_list, seq_lens):
        ...
        forward_span_vec = get_forward_segment(forward_rnn_output, s, e,
                                               get_device_of(forward_rnn_output))
        backward_span_vec = get_backward_segment(backward_rnn_output, s, e,
                                                 get_device_of(backward_rnn_output))
...
def get_conv_vecs(batch_token_repr, span_list, span_batch_size, conv_layer):
    ...
    assert len(batch_token_repr) == len(
        span_list), "the length of batch token repr is not equal to span list's length"
...
def load_weight_from_pretrained_model(model, pretrained_state_dict, prefix=""):
    ...
    for k, v in model_state_dict.items():
        ...
        for candi_name in ['.'.join(k), '.'.join(k[1:]), '.'.join(k[2:])]:
            if candi_name in pretrained_state_dict and v.size(
            ) == pretrained_state_dict[candi_name].size():
...
def clone_weights(first_module, second_module):
    ...
    if hasattr(first_module, 'bias') and first_module.bias is not None:
        first_module.bias.data = torch.nn.functional.pad(
            first_module.bias.data, (0, first_module.weight.shape[0] - first_module.bias.shape[0]),
            'constant', 0)
